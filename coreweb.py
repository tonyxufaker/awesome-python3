import asyncio, functools, inspect, os, logging
...
# 引用inspect模块，创建几个函数以获取URL处理函数和Request参数之前的关系
# 收集没有默认值的命名关键字参数
...
# 判断是否含有叫request的函数，且该参数是否为最后一个参数
...
# 从URL函数中分析需要接收的参数，从request中获取必要的参数、调用URl函数，然后把结果转化为web.Response
def RequestHandler(object):
    ...
    async def __call__(self, request):
        ...
        if kw is None:
            ...
            # 当当函数参数没有关键字参数时，剔除request除去命名关键字参数所有的参数信息
